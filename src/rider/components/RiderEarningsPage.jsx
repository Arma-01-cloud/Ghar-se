    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-[#0E382B] to-[#134E3A] text-white p-6 rounded-3xl shadow-md space-y-2">
          ...
          <p className="font-display font-black text-3xl text-white">₹{earnings.today}</p>
          <span className="text-xs text-emerald-200/80 font-semibold flex items-center gap-1">
            <ArrowUpRight className="w-4 h-4 text-emerald-400" /> {earnings.todayDeliveries} deliveries completed
          </span>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-xs space-y-2">
          ...
          <p className="font-display font-black text-3xl text-stone-900">₹{earnings.thisWeek}</p>
        ...
        <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-xs space-y-2">
          ...
          <p className="font-display font-black text-3xl text-emerald-950">₹{earnings.thisMonth}</p>
      ...
      <div className="bg-white rounded-3xl border border-stone-200 p-6 shadow-xs space-y-4">
        ...
        <div className="flex items-end justify-between gap-2 h-44 pt-4">
          {earnings.weeklyChart.map(item => (
            <div key={item.day} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
              <span className="text-[10px] font-black text-stone-700">₹{item.amount}</span>
              <div 
